...
func softwareRecipeTestCases() []recipeTestCase {
...
func createRecipeTestVM(test *testenv.Test, tc recipeTestCase, scriptKey, scriptContent string) (*gcp.VM, error) {
...
// TestSoftwareRecipeInstall verifies that the OS Config agent installs a basic SoftwareRecipe via GuestPolicy.
func TestSoftwareRecipeInstall(t *testing.T) {
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			...
			test.Step("create guest policy", func(ctx context.Context) error {
				policy, err := testenv.BuildRecipeInstallPolicy(defaultRecipeName)
				if err != nil {
					return err
				}
				_, err = test.CreateGuestPolicy(vm, policy)
				return err
			})

			test.Step("restart agent", func(ctx context.Context) error {
...
// TestSoftwareRecipeSteps verifies that the OS Config agent executes all SoftwareRecipe steps via GuestPolicy.
func TestSoftwareRecipeSteps(t *testing.T) {
	tests := softwareRecipeTestCases()
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			...
			var vm *gcp.VM
			test.Step("create VM", func(ctx context.Context) error {
				scriptKey, scriptContent := gcp.RecipeStepsStartupScript(tt.image, defaultRecipeName)
				var err error
				vm, err = createRecipeTestVM(test, tt, scriptKey, scriptContent)
				return err
			})
			...
			test.Step("create guest policy", func(ctx context.Context) error {
				policy, err := testenv.BuildRecipeStepsPolicy(defaultRecipeName, tt.pkgManager)
				if err != nil {
					return err
				}
				_, err = test.CreateGuestPolicy(vm, policy)
			...
			test.Step("verify recipe steps", func(ctx context.Context) error {
				return test.WaitForGuestAttribute(vm, gcp.GuestAttributeRecipeInstalled)
			})
		})
	}
...
// TestSoftwareRecipeMetadataPolicy verifies that the OS Config agent applies a SoftwareRecipe defined in instance metadata.
func TestSoftwareRecipeMetadataPolicy(t *testing.T) {
	tests := softwareRecipeTestCases()
