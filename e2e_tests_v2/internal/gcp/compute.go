...
// CreateGuestPolicy creates an OS Config v1beta GuestPolicy and serializes requests to avoid Spanner transaction contention.
...
func (c *Client) GetGuestAttributes(ctx context.Context, project, zone, instance, queryPath string) ([]*compute.GuestAttributesEntry, error) {
	...
	if err != nil {
		return nil, fmt.Errorf("get guest attributes %q for instance %s: %w", queryPath, instance, err)
	}
